...
public class NotificationServiceImpl implements NotificationService{
	...
	public List<NotificationResponseDTO> getNotifications(int idUserAccount) throws Exception{
		...
		for(Notification not:fetchAllNotificationsForUser) {
			NotificationResponseDTO tmpNot = new NotificationResponseDTO(not.getIdNotification(), not.getMessage(), not.getTransaction() == null?null:not.getTransaction().getTime());
			response.add(tmpNot);
		}
		return response;
	}
